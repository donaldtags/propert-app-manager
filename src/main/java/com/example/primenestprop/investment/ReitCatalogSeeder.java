 ...
 * not simulated user activity. Idempotent: only seeds when no REITs exist yet.
 */
@Component
@Order(10)
class ReitCatalogSeeder implements CommandLineRunner {
    ...
    private void seedReit(AppUser issuer, String name, String country, String propertyType, String description,
            ...
            String coverImageUrl, List<Property> backingProperties) {
        Reit reit = new Reit();
        reit.setName(name);
        ...
        reit.setProperties(new LinkedHashSet<>(backingProperties));
        reits.save(reit);
    }
}
